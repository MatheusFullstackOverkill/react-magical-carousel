...
export default class ReactMagicalCarousel extends Component<{
  ...
  async setup() {
      ...
      if(this.props.slideHeight) {
        if(this.props.slideHeight.includes('vh')) {
          const screenPercentage = parseFloat(this.props.slideHeight.replace('vh', ''));
      ...
      let slidesForwards = [initialSlides].map((x, group_index) => {
      ...
      slidesForwards = slidesForwards.map((x, index) => ({
        ...
        transition: this.props.transition ? this.props.transition/1000 + 's' : '0.3s',
        scale: this.props.applyFocus && index === 0 ? '1.2' : '1'
      }));

      const startPoint = slidesForwards[0].left;

      const reverseSlides = [...slidesForwards].reverse().map((x, index) => ({
        ...x,
  ...
  componentDidUpdate(prevProps: Readonly<Record<string, any>>, prevState: Readonly<Record<string, any>>, snapshot?: any): void {
    ...
    if (this.props.selectedSlide && 
      (
        this.props.selectedSlide.currentSlide !== prevProps.selectedSlide?.currentSlide ||
        this.props.selectedSlide.slidesJumpCount !== prevProps.selectedSlide?.slidesJumpCount
      )) {
      !this.state.block && this.onBtnClick(this.props.selectedSlide.slidesJumpCount);
    };
  }
  ...
  onSelectDot = (index: number) => {
    index - this.state.currentSlide !== 0 && this.onBtnClick(index - this.state.currentSlide);
  }
  ...
  moveSlider = async (slidesJumpCount: any) => { 
    return new Promise((resolve, _) => {
      ...
      this.setState({ slides }, () => {
        ...
        slides = [...slides].map(x => ({ ...x, transition: this.props.transition ? this.props.transition/1000 + 's' : '0.3s', }));

        this.setState({ slides }, () => {
          slides = [...slides].map((x, index) => {
            if (this.state.middleSlideFixedIndex === index) {
              this.props.onSelectSlide && this.props.onSelectSlide({ currentSlide: x.initialIndex, slidesJumpCount});
            ...
            return ({
              ...
              scale: this.props.applyFocus && this.state.middleSlideFixedIndex === index ? '1.2' : '1'
            })
          });
          ...
          <div ref={this.sliderRef} className='react-magical-carousel' style={{
            height: this.state.slideHeight + (this.props.applyFocus ? this.state.slideHeight / 100 * 20 : 0) + 'px'
              ...
              <>
                {
                  ...
                  <button className='previous jump-btn' onClick={() => this.onBtnClick(-1)}>
                ...
                }
                {
                  ...
                  <button className='next jump-btn' onClick={() => this.onBtnClick(1)}>
              ...
              <div className='navigation-dots'>
                ...
                onClick={() => this.onSelectDot(index)}></div>)}
              </div>}
          </div>
      )
  }
